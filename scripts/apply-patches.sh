#!/usr/bin/env bash
# Apply all six Hermes V1 patches in canonical order.
#
...
#     rm sample79/node_modules/react-native/.directtv-v1-patches-applied
...
RN_DIR="$REPO_ROOT/sample79/node_modules/react-native"
APP_DIR="$REPO_ROOT/sample79"
MARKER="$RN_DIR/.directtv-v1-patches-applied"

if [ ! -d "$RN_DIR" ]; then
    echo "error: $RN_DIR does not exist." >&2
    echo "       Run 'cd sample79 && npm install' first." >&2
    exit 1
fi
...
apply_patch 01-jsi-vendoring.patch "$RN_DIR"
apply_patch 02-rn-surgery.patch    "$RN_DIR"
apply_patch 04-cdp-adapter.patch   "$RN_DIR"
apply_patch 05-ios.patch           "$RN_DIR"

echo
echo "App-side patches (under $APP_DIR):"
apply_patch 03-app-side.patch       "$APP_DIR"
apply_patch 06-ios-app-side.patch   "$APP_DIR"

# Record success
{
    echo "Applied $(date -u '+%Y-%m-%dT%H:%M:%SZ') by scripts/apply-patches.sh"
    for p in 01-jsi-vendoring 02-rn-surgery 04-cdp-adapter 05-ios 03-app-side 06-ios-app-side; do
...
echo
echo "Done. All six patches in place. Marker: $MARKER"
