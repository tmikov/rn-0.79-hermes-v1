#!/usr/bin/env bash
# Apply the RN 0.85 Hermes V1 patch set.
#
...
#     rm sample85/node_modules/react-native/.directtv-v1-patches-applied
...
RN_DIR="$REPO_ROOT/sample85/node_modules/react-native"
MARKER="$RN_DIR/.directtv-v1-patches-applied"

if [ ! -d "$RN_DIR" ]; then
    echo "error: $RN_DIR does not exist." >&2
    echo "       Run 'cd sample85 && npm install' first." >&2
    exit 1
fi
